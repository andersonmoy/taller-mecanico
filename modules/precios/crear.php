...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$nom)         $error = 'El nombre del servicio es obligatorio.';
    ...
    elseif ($duracion <= 0) $error = 'La duración debe ser mayor a 0 minutos.';
        ...
        dbQuery(
            "INSERT INTO servicios (nombre, descripcion, categoria_id, precio_base, duracion_estimada, activo)
             VALUES (?, ?, ?, ?, ?, 1)",
            [$nom, $desc, $cat_id, $precio, $duracion]
        );
  ...
  <div class="content">
    ...
    <div class="inv-form-card card">
      <div class="card-header">
        <h3><i class="fas fa-screwdriver-wrench"></i> Datos del Servicio</h3>
      </div>
      <div class="card-body">
          ...
          <div class="form-grid">
            ...
            <div class="form-group">
              ...
              <input type="number" name="precio_base" class="form-control"
                     ...
                     value="<?= htmlspecialchars($_POST['precio_base'] ?? '') ?>" required>
            </div>
            <div class="form-group" style="grid-column:1/-1">
              <label class="form-group label">Descripción</label>
              <textarea name="descripcion" class="form-control" rows="2"
                        placeholder="Descripción del servicio..."><?= htmlspecialchars($_POST['descripcion'] ?? '') ?></textarea>
            </div>
          </div>
